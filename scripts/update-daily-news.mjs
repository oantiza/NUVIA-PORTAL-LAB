...
async function fetchText(url) {
  const response = await fetch(url, {
    headers: { 'user-agent': 'NUVIA-Web-3/1.0 (daily economic-news updater)' },
...
async function fetchCandidateImage(item) {
  ...
  const response = await fetch(remoteUrl, {
    headers: {
      'user-agent': 'NUVIA-Web-3/1.0 (daily economic-news updater)',
      referer: item.url,
    },
    signal: AbortSignal.timeout(30_000),
  });
...
existing.sourceRepository = 'NUVIA-WEB-3';
